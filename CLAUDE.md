...
- **완료된 마지막 버전:** `v1.2.11` (v1.0.0은 Phase B 완결 마일스톤 태그)
- **배포 완료:** v1.2.11(P1 스프린트 전체)까지 Oracle Cloud VM 배포 완료 (2026-07-14, SSH 키: `C:\Users\lg\Downloads\ssh-key-2026-07-04 (1).key`, `deploy/update.sh` — git pull cfad5f5→56cac1b(v1.2.8~v1.2.11 전체) + migrate_db 정상 + 서비스 재시작 확인, 랜딩/로그인 200 OK(VM 내부 curl), 에러 로그에 신규 트레이스백 없음(기존 봇 스캔 CSRF 경고만 존재, 무관))
- **다음 작업: `saas_plan.md` §6 실행 순서를 따를 것 (Phase A부터 순서대로)**
  ...
  23. ~~P1 스프린트 배포 (v1.2.8~v1.2.11)~~ ✅ 완료 (2026-07-14, 아래 배포 완료 항목 참고) — **다음: 신규 기능 없음, P2 착수 여부는 승헌씨 지시 대기**
  3. 이후 Phase B(CSV 임포트·요금제 3계층·연차촉진), C(성과 재개편·입사예정자), D — 상세는 saas_plan.md
  - (보류) 온보딩 투어 확장 여부, 도메인 설정(승헌씨 직접)
- **v0.91~v1.2.11 완료 내역 요약:**
