...
### 현재 진행 상태 (마지막 업데이트: 2026-07-06)

- **완료된 마지막 버전:** `v0.90.0`
- **다음 작업:** v1 잔여 항목 — 에러 스윕, UI 스윕, 문서 기능 스코핑, 온보딩 투어 나머지 메뉴(팀원 관리/채용 대시보드/분석·보고서 등) 확장 여부 결정
- **배포 완료:** v0.89.0까지 Oracle Cloud VM 배포 완료 (SSH 키: `C:\Users\lg\Downloads\ssh-key-2026-07-04 (1).key`). **v0.90.0은 로컬에만 반영, VM 미배포 (요청 시 진행)**
...
- **현재 논의 중인 내용:**
  ...
  - v0.88.0 랜딩 페이지 리뉴얼(TalentCore 디자인 시안 반영) — Oracle Cloud VM 배포 완료
  - v0.85.0 ATS→HRIS 자동화 (오퍼 수락 시 직원 자동 등록 + 온보딩 파이프라인)
  - v0.85.1 Slack DM 알림 8종 + 슬래시커맨드/인터랙티브 버튼 + APScheduler 예약 알림 + Confluence 위키 시드
  - TalentCore MCP 서버 추가 (Claude Desktop 연동), Oracle Cloud 배포 스크립트 추가 (setup/nginx/update)
...
- [x] Claude Preview로 로컬 검증 (개요 투어 5단계, 근태/성과/보상관리/채용 페이지별 투어 전부 순서·완료 처리 확인), **Oracle Cloud VM 미배포 (요청 시 진행)**
