...
### 현재 진행 상태 (마지막 업데이트: 2026-07-21)

- **완료된 마지막 버전:** `v1.6.2` (v1.0.0은 Phase B 완결 마일스톤 태그)
- **배포 완료:** v1.6.2까지 Oracle Cloud VM 배포 완료 (2026-07-21)
- **v1.6.2 — 유료화 표면 제거 (무료·오픈소스 전환, 승헌씨 지시)**: "표면만 제거" 선택 — 사용자에게 보이는 유료화 콘텐츠만 삭제하고 내부 코드(요금제 게이팅·/billing 라우트·master_db 구독테이블)는 유지. ①랜딩: 요금제 3계층 섹션+무료 파트너 배너+nav '요금제' 링크 제거 ②README: 요금제 표+BILLING_ENABLED env 예시 제거 ③약관: 제7조 '무료 제공'으로 단순화·제10조 유료 책임한도 삭제 ④방침: 결제 수집항목·전자상거래 보존기간·토스페이먼츠 위탁 삭제. 결제/구독 링크는 원래 사이드바·대시보드에 없었음(billing 페이지는 URL로만 접근, SaaS는 슈퍼어드민 전용) → 데모 노출 변화 없음. 스윕 116×3 통과, 랜딩 로컬 렌더 확인(콘솔 에러 0)
- **README 제품화 (v1.6.1~v1.6.2 사이, 2026-07-21)**: 실제 데모 화면 스크린샷 8종(docs/screenshots) + 크로스페이드·한글자막 데모 GIF(docs/talentcore-demo.gif, 3MB) 추가, MCP 연동·근로기준법 섹션 신설, 팩트 갱신(데모 URL→https, 시드 147명), MIT LICENSE 파일 신설. 스크린샷/GIF는 Playwright+Pillow로 라이브 사이트 캡처(생성 스크립트는 scratchpad, 레포 미포함)
- **v1.6.1 — 랜딩 CTA 문구 수정 (승헌씨 컨펌 완료)**: v1.6.0 랜딩 개편안 검토 결과, CTA 헤드라인 "첫 질문은 공짜입니다" → "우리 회사에 맞는 프로그램인지, 사용해보고 결정하세요"로 수정(승헌씨 피드백, 부담 없이 먼저 써보라는 톤으로 조정). 나머지 v1.6.0 내용은 그대로 컨펌됨
- **v1.6.0 — 랜딩 전면 개편 (승헌씨 컨펌 완료 → v1.6.1로 CTA 문구만 수정)**: 디자인 핸드오프(talentcore-landing-v2, AI-first 다크)를 Flask/Jinja+바닐라 JS로 구현. 자동 재생 채팅 데모, 토스·당근식 카피 재작성, 팩트 체크(가상 지표 340+기업/128,000명 등 삭제→근로기준법 자동화 6카드로 대체, MCP 조회 전용에 맞춰 "실행" 주장 제거, "유일한" 절대 표현 제거, 채용 ATS→입사 예정자, 시연 대화임을 명시). 요금제+무료 파트너 배너·가입 우선 CTA·약관/방침 푸터 유지. 레포 정리(chore, 잡동사니 46파일 삭제 + .gitignore 보강)도 같은 날 완료
- **★ 신규 로드맵 (2026-07-15 승헌씨 지시, R2→R1→R3→R4→R5 순서 확정):**
  - ~~R2: 기능 정리~~ ✅ v1.2.13 완료 — 채용 ATS Enterprise 격리(입사 예정자는 Core 유지) + 데모 테넌트 growth 강등(9박스·Talent Card·급여구조·ACR·후계자·데이터마법사 숨김, 코드 보존)
  - ~~R1: 성과관리 체계 개편~~ ✅ v1.3.0~v1.3.3 완료 (2026-07-16) — 기획서 `performance_redesign_plan.md`/`.html`(Workday·Lattice·레몬베이스 리서치, 승헌씨 승인) 기반 4단계 구현: ①매니저 '팀 성과' 사람 단위 재설계+팀원 처리 패널 ②직원 단계별 단일 화면+평가 참고 패널 ③캘리브레이션 팀 단위 재설계+Potential 게이팅 수리 ④사이클 운영(마감일·리마인드·전환 체크·결과 확인)
  - ~~R3: 보상관리 체계 개편~~ ✅ v1.4.0~v1.4.2 완료 (2026-07-16) — 기획서 `compensation_redesign_plan.html`(flex·Workday 리서치, 승헌씨 승인) 기반 3단계 구현: ①급여 운영 '이번 달 정산 보드'(3단계 사이클+검토 diff+이상 감지) ②연봉 조정 3단계(조정안→시뮬레이션→적용일 반영, 즉시 적용 폐지) ③인건비 추이+직원 기본급 타임라인. 과정에서 기존 버그 1건 수리(보상 허브 급여 생성이 근태 있는 달 500 크래시) + Compa 게이팅 잔여 2곳 수리
  - ~~R4: 출시 전 최종 보완 리스트업 + 출시 준비 기획~~ ✅ 완료 (2026-07-16) — **기획서 `launch_plan.md`/`.html` (승인 대기)**. 출시 관점 전수 감사: P0 5건(①세션 시크릿 → v1.4.3로 당일 수리·배포 완료 ②개인정보처리방침·약관 없음 ③HTTPS 미적용 ④토스 테스트 키 → 무료 파트너 모드 권장 ⑤슈퍼어드민 약한 비번) + P1 5건(SMTP·모니터링·CTA·백업 복원 리허설·메타) + P2 4건 + R5 출시 절차 10단계 + 작업 분담
  - R5: 출시 — **Claude 구현분 완료** ✅ v1.5.0~v1.5.3 (2026-07-19, launch_plan 승인 후):
    - v1.5.0 — 개인정보처리방침(/privacy)·이용약관(/terms) 신설(수탁 처리 구조·급여 참고용 고지 포함), 가입 폼 필수 동의 체크+서버 검증, 랜딩 푸터 링크. **법적 문구 최종 검토는 승헌씨 필요**
    - v1.5.1 — 무료 파트너 모드: BILLING_ENABLED 환경변수(기본 0), 구독 대시보드 파트너 히어로·₩0 청구, 카드 등록/청구 차단, 랜딩 파트너 배너. 유료 전환 시 .env에 BILLING_ENABLED=1 + 토스 라이브 키
...
- **다음 작업: `saas_plan.md` §6 실행 순서를 따를 것 (Phase A부터 순서대로)**
  ...
  16. ~~Phase C 배포 (v1.1.0~v1.2.2)~~ ✅ 완료 (2026-07-14, v1.2.3 핫픽스 포함)
  17. ~~기존 기능 전수 점검 + 개선 기획~~ ✅ 완료 (2026-07-14) — 107 라우트×3역할 스윕(500 에러 0건), 기존 버그 6건 수정, **개선 기획서: `improvement_plan.md`**
  18. ~~P0 스프린트 (v1.2.4~v1.2.7) + 배포~~ ✅ 완료 (2026-07-14) — 연차 단일화·급여 2단계·시드 정리·스윕 자동화 + 입사예정자 직원관리 통합(승헌씨 피드백). 과정에서 기존 버그 3건 추가 수정(보상 허브 급여 생성 크래시 2건 + 부양가족 계산 크래시)
  19. ~~P1-1: 승인 허브~~ ✅ v1.2.8 완료
  20. ~~P1-2: UI 한글화 스윕~~ ✅ v1.2.9 완료
  21. ~~P1-3: 결재선 확장(OT)~~ ✅ v1.2.10 완료 (복리후생 환급은 별도 작업으로 연기)
  22. ~~P1-4: CSV 왕복 (내보내기→수정→재업로드 일괄 수정)~~ ✅ v1.2.11 완료 — P1 스프린트 전체 완료
  23. ~~P1 스프린트 배포 (v1.2.8~v1.2.11)~~ ✅ 완료 (2026-07-14, 아래 배포 완료 항목 참고)
  24. ~~P2-1: 모바일 3대 동선 검증(체크인/아웃·휴가신청·급여명세)~~ ✅ 완료 (2026-07-15, Claude Browser 375~378px 뷰포트 실기기 검증, 콘솔 에러 0건)
  25. ~~P2-2: 성과관리 팀원 검색/필터~~ ✅ 완료 (2026-07-15)
  26. ~~P2-3: UI 한글화 잔여분 정리 + 알림 버그 수정~~ ✅ 완료 (2026-07-15)
  27. ~~P2-4: 온보딩 투어 확장~~ ✅ 완료 (2026-07-15) — P2 스프린트 전체 완료
  28. ~~P2 스프린트 배포 (v1.2.12)~~ ✅ 완료 (2026-07-15, 아래 배포 완료 항목 참고) — **다음: 신규 기능 없음, 다음 착수 여부는 승헌씨 지시 대기**
  3. 이후 Phase B(CSV 임포트·요금제 3계층·연차촉진), C(성과 재개편·입사예정자), D — 상세는 saas_plan.md
  - (보류) 도메인 설정(승헌씨 직접)
- **v0.91~v1.3.3 완료 내역 요약:**
  - v1.3.3 — **성과 개편 R1-D: 사이클 운영**: performance_cycles에 goal_deadline/review_deadline + calibration_results에 acknowledged_at (마이그레이션). 주기 생성·관리에서 단계별 마감일 설정/인라인 수정, 성과 배너 D-day 배지(경과 시 경고). 미완료 리마인드 버튼(목표 단계=미확정자·미작성자, 평가 단계=자기평가 미제출자, 매니저는 부서 스코프, 감사 로그). 주기 관리에 단계별 미완료 현황 상시 표시 + 전환 시 경고 flash. 결과 확인(Acknowledgement) — 직원 [결과 확인] 클릭 시 acknowledged_at 기록, HR 확인률 추적(Workday 패턴). migrate_db가 테넌트 DB 전체(tenant_*.db) 마이그레이션하도록 보강. 테스트 7단계 + 스윕(111×3) 통과
  - v1.3.2 — **성과 개편 R1-C: 캘리브레이션 팀 단위 재설계**: 153행 플랫 테이블 → 좌측 부서 목록(확정률 n/n·완료 ✓) + 우측 본문(전체 요약 뷰=전체 분포 바(권장 배분 % 병기)+부서별 진행 현황 [조정하기] 딥링크 / 부서 상세 뷰=부서 분포 바+종합점수 내림차순 테이블). Potential 배너·컬럼 Enterprise 게이팅(growth 노출 위반 수리). 잠재 버그 2건 수리 — 확정 행의 잠재력/하향사유가 라우트에서 안 실려 항상 '—'였던 것, 재확정 시 잠재력 소실(COALESCE upsert). 테스트 5단계 + 스윕 통과
  - v1.3.1 — **성과 개편 R1-B: 직원 '내 성과' 단계별 단일 화면**: 3탭(내 목표/다면평가/평가 결과) 병렬 → 단계 스위칭 — goal~calibration 단계는 목표·자기평가가 본문(동료 평가는 review 단계에만 섹션), appeal/closed 단계는 결과·이의신청이 본문(목표는 아코디언), calibration 단계 "결과 공개 대기" 안내. 다면평가 작성 화면(peer_write) 우측에 피평가자 확정 목표 참고 패널(레몬베이스 패턴). 실브라우저(직원 2계정)+테스트클라이언트 검증 + 스윕 통과
  - v1.3.0 — **성과 개편 R1-A: 매니저 '팀 성과' 사람 단위 재설계**: 200개 목표 카드 평면 나열 → 사람 단위 테이블(목표 수·가중치/목표 상태/진행률/자기평가/내 평가/등급/처리 버튼) + 단계별 처리 현황 칩 + 목표 미작성 팀원도 표시. 팀원 상세 처리 패널 신설(`/performance/team/<id>`) — 목표 승인·반려+목표별 평가 일괄 입력(upsert)을 한 화면에, 우측 참고 패널(진행률·자기평가·지난 등급), [저장 후 다음 팀원] 순차 처리, 매니저 소유권 검증. 기획서 performance_redesign_plan.md/.html 작성(리서치 출처 포함). 테스트 9단계 + 스윕 통과
  - v1.2.13 — **기능 정리: 채용 ATS Enterprise 격리 + 데모 growth 강등 (R2)**: 채용 ATS(공고/파이프라인/면접/오퍼/채용요청서)를 `_GROWTH_FEATURES`→`_ENTERPRISE_FEATURES`로 이동(입사 예정자는 Core 유지), 데모 테넌트(1) plan enterprise→growth(로컬+VM master.db 직접 UPDATE, master_db.py 신규 시드도 growth). 게이팅 누락 진입점 수리 — admin 대시보드 채용 스탯카드→입사예정자 카드/포지션 위젯/공고등록 퀵액션→입사예정자, 글로벌 검색 채용 2항목+입사예정자 추가, Talent Card 링크 2곳(성과 매니저 뷰·직원상세), 내보내기 허브 채용 섹션, 보상 허브 ACR·Compa KPI→확정대기초안·평균기본급 대체(avg_base_salary 신규), recruiter 역할 빈 관리자 섹션 숨김, 랜딩 요금제 카피 정합. saas_plan.md §2에 결정 변경 기록. 스윕(110×3) 통과 + 데모 실브라우저 8화면 검증
  - v1.2.12 — **P2 스프린트: 성과 팀원 필터 + 알림 정리 + 온보딩 투어 확장 (improvement_plan.md)**: ①성과관리 매니저 뷰에 팀원 이름 검색창 + "승인·평가 미완료만 보기" 체크박스 필터(JS, 서버 왕복 없음) 추가. ②잔여 영문 UI 문자열 한글화 — 사이드바 글로벌 검색 항목, 역할 라벨(HR 관리자/매니저/채용담당자/게스트), 에러 페이지(403/404), 알림 페이지, 휴가 신청 폼 등. `add_notification()` 호출 인자 개수 불일치로 채용요청서 승인 알림이 조용히 실패하던 버그 수정(`requisition_dept_approve`/`requisition_hr_approve`), 채용요청서 제출 로직을 `_requisition_submit_for_approval()` 헬퍼로 통합(중복 제거). ③온보딩 투어를 성과 주기 관리(단계 스텝퍼)·회사 설정(결재선)·직원 관리(재직/입사예정/퇴직 탭) 3개 화면으로 확장. ④P2-1 모바일 3대 동선(체크인/아웃·휴가신청·급여명세) 375~378px 뷰포트 실기기 검증 — 휴가신청 실제 제출까지 확인 후 테스트 데이터 정리. 스윕(110라우트×3역할) 통과
  - v1.2.11 — **CSV 왕복 — 내보내기→수정→재업로드 일괄 수정 (P1-4, improvement_plan.md)**: 직원 목록 상단 'CSV 일괄 수정' 버튼 신규. `/employees/export-editable`: 재직 직원을 사번 키로 8개 컬럼(사번/이름/부서/직급/직군/고용형태/기본급(월)/매니저이메일) CSV 내보내기(이름은 참고용, export_ 네이밍 규칙으로 자동 감사로그). `/employees/bulk-update` 2단계 플로우 — 업로드 시 사번으로 매칭 후 **변경된 필드만** 진단(부서·직급·직군 이름 매칭 실패/고용형태 오타/기본급 숫자 아님/매니저 이메일 없음은 행별 오류로 표시, 나머지는 그대로 진행), 빈 칸은 '변경 없음'으로 처리(일괄 삭제 방지), 변경 없는 행·오류 행은 반영 대상에서 제외해 요약 카드 3종(반영예정/변경없음/오류)으로 표시. `/employees/bulk-update/confirm`: 부서·직급·직군·고용형태·매니저는 직접 UPDATE, 급여는 `salary_history` 기록 병행(사유='CSV 일괄 수정'), 감사 로그 남김. 토큰 기반 임시 파일은 기존 CSV 임포트와 같은 `IMPORT_TMP_DIR` 재사용(파일명 프리픽스로 충돌 방지), confirm 시 토큰 alnum 검증(경로 조작 방지). 실제 Flask 테스트클라이언트로 내보내기→수정→미리보기→확정 전 과정 라이브 검증(DB 반영·salary_history·audit_logs 모두 정상 확인 후 되돌림). 스윕(110라우트×3역할) 통과
  - v1.2.10 — **결재선 확장: 연장근로(OT) 승인 (P1-3, improvement_plan.md)**: 기존 결재선 프레임워크(v1.2.2, 휴가/증명서/발령)에 OT 편입 — 회사 설정 결재선 탭에 `overtime` 항목 추가(매니저 전결/매니저→HR 2단계, 제네릭 렌더링이라 템플릿 변경 없이 자동 노출). `overtime_requests`에 manager_id/manager_approved_at/hr_id/hr_approved_at 컬럼 + status `reviewed` 추가(마이그레이션). `overtime_approve`/`overtime_reject`를 `attendance_approve`/`attendance_reject` 패턴으로 전면 재작성 — 매니저 소유권 검증(직속 부하 또는 같은 부서) 신규 추가(기존엔 매니저면 타 부서 OT도 승인 가능했던 구멍). 결재 대기함·근태 홈 승인 대기 목록에 "매니저 검토완료 — HR 대기" 상태 반영. **부수 버그 수정**: 기존 OT 승인/반려 코드의 `add_notification()` 호출 인자 개수 불일치로 실행할 때마다 TypeError 나던 것(사실상 한 번도 정상 동작한 적 없던 상태) 수정. 스윕(108라우트×3역할) 통과
  - v1.2.9 — **UI 한글화 스윕 (P1-2, improvement_plan.md)**: 국내 컴플라이언스 SaaS 포지셔닝인데 화면마다 영문이 혼재하던 문제 — 템플릿 전수 Grep 스윕(공지사항/퇴직 요청함/채용 대시보드·공고·지원자·오퍼레터/급여관리(연도·월·직원급여표·4대보험 요율·수정모달)/성과관리(잠재력 점수 Low·Mid·High→낮음·중간·높음, Talent Card·캘리브레이션 표기 통일)/연동관리/증명서 허브 등 총 33개 템플릿) 전면 한글화. **유지한 관례(의도적 예외)**: 브랜드명(TalentCore), 기술용어(Compa-Ratio·Min/Mid/Max 급여밴드·HR 약어), 프레임워크명(피어리뷰 Start/Stop/Continue), 외부 서비스 브랜드명(Slack/Jira/Confluence), 랜딩페이지 마케팅 카피(Product Tour 모듈명·Claude 언급·Core/Growth/Enterprise 요금제명)는 기존 설계 결정으로 그대로 유지. 스윕(108라우트×3역할) 통과
  - v1.2.8 — **승인 허브 (P1-1, improvement_plan.md)**: ①`/approvals` 결재 대기함 신설(manager_or_admin) — 휴가·근태(HR은 reviewed 포함)/연장근로/목표 승인/등급 이의신청 + admin 전용 증명서/인사발령/퇴직/급여 초안(월별)/입사 예정 D-7을 한 화면에, 유형 필터 칩 + 접수 시각 + '처리하기' 딥링크(휴가→근태 승인 탭, 목표→해당 주기 성과 페이지, 발령→직원 상세 #hr 등). 매니저는 부서/직속 범위만. ②admin 대시보드 인박스에 신규 5종 편입(목표 승인 3·이의 3·OT 3·입사 D-7 3·급여 초안 2건 상한) + '결재 대기함 전체' 링크. ③매니저 대시보드 인박스에 목표 승인/이의/OT 편입 — 항목별 액션 분기(휴가=승인/반려 인라인, 나머지=확인 링크), Approve/Reject 버튼 한글화. ④사이드바 팀 섹션 최상단 '결재 대기함'. 부수: tests/sweep.py 직원 계정을 emp002로 변경(employee@company.com이 v1.2.6에서 manager 승격됨). 테스트 6케이스 + 스윕(108라우트) 통과
  - v1.2.7 — **입사예정자→직원 관리 통합 + 목표 템플릿 메뉴 정리 (승헌씨 UX 피드백)**: 직원 목록 상단에 [재직 직원|입사 예정 N|퇴직] 탭 신설 — employees 라우트에 status 파라미터(active/resigned) + 탭 카운트, /hires 화면에도 동일 탭 바(active_page='employees'로 사이드바는 팀원 관리 하이라이트, 기존 URL·알림 링크 호환 유지). 사이드바 단독 메뉴 2개 제거(입사 예정자, 목표 템플릿). 목표 템플릿 진입점은 성과 페이지 상단 버튼(매니저/관리자)으로 이동 — 목표 등록 폼의 '템플릿에서 가져오기'는 그대로. 테스트 6케이스 + 스윕 통과
  - v1.2.6 — **시드 정리 + 스윕 자동화 (P0-3+4)**: ①`tests/sweep.py` 신규 — 107 GET 라우트 × 3역할 전수 스윕을 저장소에 편입, 릴리즈 워크플로우 1단계로 승격(계정은 SWEEP_* 환경변수로 오버라이드 가능). ②시드 매니저 role 정합성 — `_fix_manager_roles()`(멱등, migrate_db 매 실행 시): 직속 부하가 있는 employee 23명을 manager로 승격 (기존엔 manager role이 1명뿐이라 승인 데모가 어색했음). ③tenant_None.db 원인 조사 — 현행 코드에 재현 경로 없음(6/10 멀티테넌시 구축 초기 잔재로 판단), `get_tenant_db_path()`에 None/비정상 tenant_id → 테넌트 1 폴백 + 경고 로그 가드 추가, 잔재 파일 삭제
...
10. **작업 완료 후 릴리즈 워크플로우** — 매 작업마다 아래 순서를 반드시 따른다:
    1. **전 라우트 스윕 통과 확인** — `python tests/sweep.py` (107 GET 라우트 × 3역할, 500 에러 0건이어야 함. 실제로 v1.2.2~v1.2.5에서 잡은 버그 유형: 없는 컬럼/엔드포인트/Row 메서드 참조)
    2. `git add` + `git commit -m "vX.X.X — 작업 내용"`
    3. `git push`
    4. `git tag vX.X.X` + `git push --tags`
    5. `gh release create vX.X.X --title "vX.X.X — 제목" --notes "내용"`
    6. **서버 재시작** — 기존 python 프로세스 kill 후 `python run.py` 백그라운드 실행
    7. 사용자에게 **바뀐 점 요약** + **테스트 체크리스트** 제공
