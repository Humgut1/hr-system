...
- **완료된 마지막 버전:** `v1.2.7` (v1.0.0은 Phase B 완결 마일스톤 태그)
...
- **다음 작업: `saas_plan.md` §6 실행 순서를 따를 것 (Phase A부터 순서대로)**
  ...
  18. ~~P0 스프린트 (v1.2.4~v1.2.7) + 배포~~ ✅ 완료 (2026-07-14) — 연차 단일화·급여 2단계·시드 정리·스윕 자동화 + 입사예정자 직원관리 통합(승헌씨 피드백). 과정에서 기존 버그 3건 추가 수정(보상 허브 급여 생성 크래시 2건 + 부양가족 계산 크래시) — **다음: P1 스프린트 (승인 허브→한글화→결재선 확장→CSV 왕복. 승인 허브는 Fable 권장, 한글화부터 Sonnet 전환 가능) 또는 Phase D**
  3. 이후 Phase B(CSV 임포트·요금제 3계층·연차촉진), C(성과 재개편·입사예정자), D — 상세는 saas_plan.md
  - (보류) 온보딩 투어 확장 여부, 도메인 설정(승헌씨 직접)
- **v0.91~v1.2.7 완료 내역 요약:**
