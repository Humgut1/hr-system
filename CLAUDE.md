...
> Workday HCM 기반 TalentCore 개편 계획 (talentcore_plan.html 기준)
> 우선순위: 성과관리 완성 → 보상 연동 → 채용 고도화 → 나머지

---

### Phase 1 — 성과관리 완성 (v0.48 ~ v0.51)

#### v0.48.0 — 피어리뷰 UX 개선
- [ ] `peer_write.html` — Start / Stop / Continue 프레임워크 (Strengths→Stop, Growth→Start, 신규→Continue)
- [ ] `app.py` — 익명성 임계값: 3명 미만 응답 시 집계 결과 비공개 처리
- [ ] `peer_reviews.html` — 결과 표시 시 응답자 수 임계값 체크

#### v0.49.0 — 캘리브레이션 고도화
- [ ] `calibration.html` — 하향 시 사유 필수 입력 (JS 감지 + 서버 검증)
- [ ] `app.py` — 최대 1단계 하향 제한 로직 (평균 점수 기준)
- [ ] `database.py` — calibration_results에 potential_score 컬럼 추가
- [ ] `calibration.html` — Potential Score 입력 UI (Low/Mid/High)
- [ ] `calibration.html` — Forced Distribution 비율 패널 (현재 배분 vs 목표)

#### v0.50.0 — 9박스 캘리브레이션 UI
- [ ] `templates/performance/calibration_9box.html` — 신규: 드래그앤드롭 9박스 그리드
- [ ] `app.py` — /performance/calibration/9box GET/POST 라우트
- [ ] `database.py` — calibration_results에 box_position 컬럼 (1~9)

#### v0.51.0 — Talent Card + 등급 히스토리
- [ ] `templates/performance/talent_card.html` — 신규: 직원 종합 카드
- [ ] `app.py` — /performance/talent-card/<user_id> 라우트
- [ ] `templates/employees/detail.html` — 성과 탭에 등급 히스토리 타임라인

---

### Phase 2 — 보상 연동 강화 (v0.52 ~ v0.53)

#### v0.52.0 — Merit Matrix + 보상 투명성
- [ ] `database.py` — merit_matrix 테이블 (grade, compa_band, suggested_pct)
- [ ] `app.py` — bulk-raise에서 캘리브레이션 등급 기반 인상률 자동 제안
- [ ] `templates/admin/settings.html` — 등급별 인상률 배수 설정 UI
- [ ] `templates/performance/index.html` — 직원에게 등급별 보상 배수 사전 공개

#### v0.53.0 — Compa-Ratio + Pay Equity
- [ ] `database.py` — salary_grades에 market_midpoint 컬럼
- [ ] `app.py` — Compa-Ratio 자동 계산 (실제급여 / 시장중간값)
- [ ] `templates/analytics/index.html` — Compa-Ratio 분포 차트 추가

---

### Phase 3 — 채용 고도화 (v0.54 ~ v0.55)

#### v0.54.0 — 채용 Business Process
- [ ] 채용 단계별 승인 워크플로 (JD 기안 → HR 승인 → 오퍼 승인)
- [ ] 지원자 스킬 매칭 점수

#### v0.55.0 — 후계자 계획
- [ ] `database.py` — succession_plans 테이블
- [ ] `templates/performance/succession.html` — 포지션별 후보 리스트

---

### Phase 4 — 인프라 (언제든)
- [ ] 직원별 복리후생 오버라이드 UI (/employees/<id>/benefits)
- [ ] 토스페이먼츠 실제 키 연동 테스트
- [ ] 목표 템플릿 (부서/직군별 표준 OKR)
