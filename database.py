...
def init_db(db_path: str = None):
    ...
    try:
        ...
        # 2026년 한국 공휴일 시드 (관공서 공휴일에 관한 규정 · 대체공휴일 포함)
        # 입사일 규칙(월·수, 공휴일 제외)이 이 표를 쓰므로 대체공휴일이 빠지면 쉬는 날이 입사일로 열린다.
        holidays_2026 = [
            ('2026-01-01', '신정'),
            ('2026-02-16', '설날 연휴'),
            ('2026-02-17', '설날'),
        ...
        # 예전 시드가 넣은 틀린 날짜(2025년 설날 날짜·부처님오신날)는 이름까지 같을 때만 지운다.
        for hdate, hname in (('2026-01-28', '설날 연휴'), ('2026-01-29', '설날'),
                             ('2026-01-30', '설날 연휴'), ('2026-05-15', '부처님오신날')):
            c.execute('DELETE FROM public_holidays WHERE date=? AND name=?', (hdate, hname))
